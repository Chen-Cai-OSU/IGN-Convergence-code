...
class DataGenerator:
    ...
    def load_data(self):
        ...
        graphs, labels = helper.new_load_dataset(self.config) # changed by Chen
        ...
        # added by Chen
        self.graphs = [np.expand_dims(g, 0) for g in graphs]
        self.labels = labels
        self.val_size = len(self.graphs)
    ...
    def new_initialize(self):
        self.iter = zip(self.graphs, self.labels)
...
if __name__ == '__main__':
    # import utils
    # print(utils)
    config = config.process_config('../configs/example.json')
    data = DataGenerator(config)

    data.initialize(True)
    for g, label in data.iter:
        print(g.shape, label)
    print()
